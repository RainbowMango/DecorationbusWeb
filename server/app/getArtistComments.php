...
$target_artist_id = 0; //所要查询的设计师
$artistCommentList = array('total'=>0, 'row'=>array()); //默认返回结果
...
if(!isset($_GET["artist"])) {
    return json_encode($artistCommentList);
}
$target_artist_id = $_GET["artist"];

//查询
function getArtistComments($counter) {
    global $artistCommentList;
    global $target_artist_id;
    ...
    $result = $conn->query("select usernickname, useravatar, commentdate, comment, score, thumbnail01, thumbnail02, thumbnail03, thumbnail04, image01, image02, image03, image04 from decoration_artist_comments where artistid=$target_artist_id LIMIT $limit_start, $limit_stop ");
    if (!$result) {
        throw new Exception('Search user score failed.');
    }

    $num_result = $result->num_rows;
    $artistCommentList['total'] = $num_result;
    ...
    for($i = 0; $i < $num_result; $i++) {
        ...
        $artistCommentList["row"][$i] = $converted_row;
    }
...
$jsonStr = getArtistComments($param_counter);
print("$jsonStr");
