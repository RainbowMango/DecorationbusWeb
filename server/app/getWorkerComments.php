...
$target_worker_id = 0; //所要查询的设计师
$workerCommentList = array('total'=>0, 'row'=>array()); //默认返回结果
...
if(!isset($_GET["worker"])) {
    return json_encode($workerCommentList);
}
$target_worker_id = $_GET["worker"];

//查询
function getWorkerComments($counter) {
    global $workerCommentList;
    global $target_worker_id;
    ...
    $result = $conn->query("select usernickname, useravatar, commentdate, comment, score, thumbnail01, thumbnail02, thumbnail03, thumbnail04, image01, image02, image03, image04 from decoration_worker_comments where workerid=$target_worker_id LIMIT $limit_start, $limit_stop ");
    if (!$result) {
        throw new Exception('Search user score failed.');
    }

    $num_result = $result->num_rows;
    $workerCommentList['total'] = $num_result;
    ...
    for($i = 0; $i < $num_result; $i++) {
        ...
        $workerCommentList["row"][$i] = $converted_row;
    }
...
$jsonStr = getWorkerComments($param_counter);
print("$jsonStr");
